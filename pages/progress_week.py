...
class Progress_Week(UserControl):
    ...
    def build(self):
        ...
        if count1 is not None:
            # Calculate the start and end dates for the current week (from Sunday to Saturday)
            today = datetime.now()
            start_date = today - timedelta(days=today.weekday())
            end_date = start_date + timedelta(days=6)
            
            # Format the dates as strings in the "%Y-%m-%d" format
            ...
            # print(df_merged)
            # Plot the bar graph
            plt.figure(figsize=(len(df_merged) * 1.0, 6))
            plt.bar(np.arange(1, len(df_merged) + 1), df_merged['Correct_Reps'], width=0.5, color='skyblue')
            plt.title('BICEP CURL', fontsize=20)
            plt.ylabel('Correct Reps')
            plt.xlabel("Week")
            plt.xticks(np.arange(1, len(df_merged) + 1), ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'])
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            plt.tight_layout()
            plt.savefig('bicep_week.png')
            plt.close()
        ...
        graph_container = ft.Container(
            ft.Row([
                ft.Image(src="bicep_week.png", height=320, fit=ft.ImageFit.CONTAIN),
                ft.Image(src="squat_week.png", height=320, fit=ft.ImageFit.CONTAIN),
            ]),
